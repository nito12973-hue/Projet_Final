...
| 16 | ~~Rendre utile l'encart "gouvernance" du dashboard admin~~ | **FAIT** (commit 1285147) : décision utilisateur (2026-08-03) — comptes désactivés (rendus cliquables, filtre déjà existant) + dernières connexions admin (nouveau panel, `User.last_login`, 5 plus récentes, exclut les admins jamais connectés). | Faible-moyenne | 1 j | dashboard.html, views.py |
...
| 23 | Captures réelles des dashboards (section Services) | **Partiellement fait** (commit 0538f86) : 3 des 6 cartes Services ont un mini-visuel CSS/SVG pur décoratif (motif QR, répartition assurance/patient, avatars famille) — pas de vraie capture d'écran, aucune n'existe. Le volet "showcase des 4 dashboards" (item 22) a été livré en Phase 3 (commit c95bd25) ; il reste ouvert uniquement sur le volet "vraie capture d'écran", qui n'existe toujours pas. | Moyenne | 2 j | landing.html |
| 24 | ~~Mention conformité RGPD / hébergement santé~~ | **FAIT** (commit e1fd315, corrigé en revue finale) : aucune certification affirmée (aucune n'existe) — uniquement des pratiques réellement implémentées (accès par rôle, pas d'inscription publique, QR unique, tentatives de connexion limitées), avec une ligne de disclosure explicite et visible indiquant qu'aucune certification formelle (hébergement de données de santé, audit RGPD) n'est en place à ce jour. La version livrée initialement disait à tort « en cours d'évaluation », ce qui suggérait un processus de certification actif ; corrigé en négation explicite lors de la revue finale de branche. | Faible (texte) | 0,5 j | landing.html |
| 25 | ~~Page contact commercial / processus de vente~~ | **FAIT** (commit e1fd315) : nouvelle section "Devenir partenaire", distincte de la section "Accès" existante (qui couvre la connexion d'un utilisateur déjà provisionné) — couvre l'engagement d'une organisation (assurance, IPM, hôpital, clinique, pharmacie, entreprise) en 3 étapes. | Faible-moyenne | 1 j | landing.html |
...
| 29 | ~~Mode "carte" responsive pour tableaux mobiles clés~~ | **FAIT** (commit 9286d09) — livré avant cette session, la ligne du tableau n'avait pas été mise à jour. | Moyenne-élevée | 3 j | mes_rendez_vous.html, mes_ordonnances.html |
